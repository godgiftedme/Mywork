...
if ($Script -eq 1) {
    [String]$GUIRunOnce = Read-Host ("Please provide Command to run once")
}
if ($Script -eq 2) {
...
foreach ($c in $count) {
    ...
    if ($Group -eq 1) {
        $CustomSpec = New-OSCustomizationSpec -Name "$CustName" -OSType Windows -FullName "$OwnerName" -OrgName "$OrgName" -Workgroup "$Workgroup" -AdminPassword "$AdminPwd" -ChangeSid -AutoLogonCount 2 -TimeZone "$Timezone" -GuiRunOnce "$GUIRunOnce"
    }
    if ($Group -eq 2) {
        $CustomSpec = New-OSCustomizationSpec -Name "$CustName" -OSType Windows -FullName "$OwnerName" -OrgName "$OrgName" -Domain "$Domain" -DomainUsername "$Domainusername" -DomainPassword "$DomainPwd"  -AdminPassword "$AdminPwd" -ChangeSid -AutoLogonCount 2 -TimeZone "$Timezone" -GuiRunOnce "$GUIRunOnce"
    }
